...
namespace PortafolioEPIS.Areas.AreaDocente.Controllers.Informes
{
    ...
    public class InformeFinalDocenteController : Controller
    {
        ...
        public ActionResult Guardar1(Tbl_InformeFinal objInformeFinal, int idprueba, int psilabo, int prarealizadas, int elaboratorio, int codigo, string estado, int prorealizados, int matriculados, int estretiro, int estabandono, int estasistentes, int estaprobados, int estdesaprobados, int notaalta, int notapromedio, int notabaja)
        {
            ...
            return Redirect("~/AreaDocente/InformeFinalDocente/Agregar/" + codigo);
        }
        ...
        public ActionResult AgregarObservaciones(int idInformeFinal = 0, int idPruebaDoc = 0, int codDetalle=0)
        {
            ...
            //ViewBag.ListaTbl_MedidasCorrectivas = objlistaMedidasCorrectivas.Listar(); cccc
            return View(
        ...
        public ActionResult Guardar(Tbl_InformeFinal objInformeFinal)
        {
            ...
            {
                objInformeFinal.Guardar();
                return Redirect("~/InformeFinalDocente");
            }
            else
            {
                return View("~/AreaDocente/InformeFinalDocente/Agregar.cshtml");
            }
